...
export function ResearchTable() {
  ...
  function handleClearAll() {
    ...
    if (
      confirm(`Clear all ${entries.length} entries? This cannot be undone.`)
    ) {
      clearEntries();
      setSelected(new Set());
    }
  ...
  function handleExport(format) {
    ...
    if (format === "json") {
      ...
      downloadFile(
        JSON.stringify(prepareForExport(data), null, 2),
        `outlier_${date}.json`,
        "application/json",
      );
    } else {
    ...
    input.onchange = (e) => {
      ...
      r.onload = (ev) => {
        try {
          ...
          if (confirm(`Import ${data.length} entries? Replaces current data.`))
            importEntries(data);
        } catch (err) {
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <Select value={exportMode} onValueChange={setExportMode}>
            ...
            <SelectContent>
              ...
              <SelectItem value="qualified" className="text-[10px]">
                Qualified Only
              </SelectItem>
              <SelectItem value="filtered" className="text-[10px]">
                Current Filter
              </SelectItem>
            </SelectContent>
          </Select>
        ...
        {rows.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 text-muted-foreground gap-2">
            ...
            <span className="text-xs tracking-widest">
              NO ENTRIES — FETCH OR ENTER DATA
            </span>
          </div>
        ) : (
          <table
              ...
              {rows.map((e, i) => (
                ...
                >
                  ...
                  <td style={{ width: colWidths.qual }} className="px-2 py-1.5">
                    <Badge
                      variant={e.qualifies ? "success" : "destructive"}
                      className="text-[9px]"
                    >
                      {e.qualifies ? "YES" : "NO"}
                    </Badge>
                  ...
                  <td
                    style={{ width: colWidths.searchTerms, overflow: "hidden" }}
                  ...
                  <td
                    style={{ width: colWidths.titleType }}
                    className="px-2 py-1.5 text-[10px] text-primary"
                  >
                    {e.titleType || "—"}
                  </td>
                  <td
                    style={{ width: colWidths.emotion }}
                    className="px-2 py-1.5 text-[10px] text-muted-foreground"
                  >
                    {e.emotion || "—"}
                  </td>
                  <td
                    style={{ width: colWidths.hook }}
                    className="px-2 py-1.5 text-[10px] text-muted-foreground"
                  >
                    {e.hook || "—"}
                  </td>
                  <td
                    style={{ width: colWidths.pacing }}
                    className="px-2 py-1.5 text-[10px] text-muted-foreground"
                  >
                    {e.pacing || "—"}
                  </td>
