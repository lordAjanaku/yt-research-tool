    ...
    for (const item of (d.items || [])) {
      ...
      if (includeReplies && item.replies?.comments) {
        for (const reply of item.replies.comments) {
          ...
          allComments.push({
            id: reply.id,
            text: rs.textDisplay.replace(/<[^>]*>/g, '').trim(),
            likes: rs.likeCount || 0,
            author: rs.authorDisplayName,
            date: rs.publishedAt?.slice(0, 10) || '',
            isReply: true,
          })
        }
      }
...
export function PatternAnalysis() {
  ...
  // ── OUTLIER MODE STATE ──
  const [outlierSource, setOutlierSource] = useState('store') // 'store' | 'upload'
  const [uploadedEntries, setUploadedEntries] = useState([])
  const [outlierFilter, setOutlierFilter] = useState('qualified') // 'all' | 'qualified'
  ...
  async function runOutlierAnalysis() {
    if (!outlierData.length) { setOutlierError('No data to analyse. Add entries or upload a CSV.'); return }
    ...
    try {
      ...
      setOutlierError('Analysis failed: ' + e.message)
    }
  ...
  async function handleFetchComments() {
    ...
    if (!ytApiKey) { setCommentFetchLog('✗ No YouTube API key configured. Go to API Keys settings.'); setCommentFetchLogType('err'); return }
    setCommentFetching(true)
    setAllComments([])
    setCommentResult(null)
    setCommentFetchLog('Fetching comments...')
    setCommentFetchLogType('info')
    try {
      const comments = await fetchYTComments(
        commentURL.trim(),
        parseInt(commentMax) || 100,
        commentReplies === 'yes',
        ytApiKey
      )
      setAllComments(comments)
      const minL = parseInt(commentMinLikes) || 0
      const filtered = comments.filter(c => c.likes >= minL)
      setCommentFetchLog(`✓ ${comments.length} comments fetched. ${filtered.length} pass the likes filter.`)
      setCommentFetchLogType('ok')
    } catch (e) {
      setCommentFetchLog('✗ ' + e.message)
      setCommentFetchLogType('err')
    }
    setCommentFetching(false)
  }

  async function runCommentAnalysis() {
    if (!filteredComments.length) { setCommentError('No comments to analyse. Fetch comments first.'); return }
    if (!aiApiKey) { setCommentError('No AI API key configured. Go to API Keys settings.'); return }
    setCommentRunning(true)
    setCommentError('')
    setCommentResult(null)
    try {
      const result = await callAI(aiProvider, aiApiKey, buildCommentSystemPrompt(), buildCommentUserMessage(filteredComments))
      setCommentResult(result)
    } catch (e) {
      setCommentError('Analysis failed: ' + e.message)
    }
  ...
  function exportCommentCSV() {
    ...
    const rows = allComments.map(c =>
      [c.likes, c.isReply ? 'reply' : 'top', c.date, `"${c.author?.replace(/"/g,'""')}"`, `"${c.text?.replace(/"/g,'""')}"`].join(',')
    )
          ...
          <div style={{ width: PANEL_WIDTH, boxSizing: 'border-box' }} className="p-3 flex flex-col gap-3">
            ...
            <div className="border border-border p-3 flex flex-col gap-1" style={{ width: '100%', boxSizing: 'border-box' }}>
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge variant={aiApiKey ? 'success' : 'destructive'} className="text-[9px]">
                  {aiApiKey ? 'KEY SET' : 'NO KEY'}
                </Badge>
              </div>
              <Badge variant={ytApiKey ? 'success' : 'destructive'} className="text-[9px] w-fit">
                YouTube API: {ytApiKey ? 'Connected' : 'Not set'}
              </Badge>
              {(!aiApiKey || !ytApiKey) && (
                <p className="text-[9px] text-muted-foreground mt-1">Configure keys in API Keys settings.</p>
              )}
            </div>

            {/* SECTION 1 — OUTLIER DATA */}
            <div className="border border-primary/30 bg-card" style={{ width: '100%', boxSizing: 'border-box' }}>
              ...
              <div className="p-3 flex flex-col gap-3" style={{ width: '100%', boxSizing: 'border-box' }}>
                ...
                {outlierSource === 'upload' && (
                  <div className="flex flex-col gap-1">
                    ...
                    <input type="file" accept=".csv,.json"
                      ...
                      className="text-[10px] text-muted-foreground file:mr-2 file:py-1 file:px-2 file:border file:border-border file:bg-card file:text-[10px] file:text-foreground file:cursor-pointer w-full" />
                    {uploadedEntries.length > 0 && (
                      <p className="text-[9px] text-green-400">{uploadedEntries.length} entries loaded</p>
                    )}
                  </div>
                )}
                <div className="flex flex-col gap-1">
                ...
                <Button className="w-full text-xs" onClick={runOutlierAnalysis}
                  disabled={outlierRunning || !outlierData.length}>
                  {outlierRunning ? 'Analysing...' : 'Run Outlier Analysis'}
                </Button>
                {outlierRunning && <Progress value={undefined} className="h-1 w-full animate-pulse" />}
              </div>
            </div>

            {/* SECTION 2 — COMMENT FETCH */}
            <div className="border border-primary/30 bg-card" style={{ width: '100%', boxSizing: 'border-box' }}>
              <div className="px-3 py-2 border-b border-primary/20 bg-primary/5">
              ...
              <div className="p-3 flex flex-col gap-3" style={{ width: '100%', boxSizing: 'border-box' }}>
                <div className="flex flex-col gap-1">
                  ...
                  <Input value={commentURL} onChange={e => setCommentURL(e.target.value)}
                    placeholder="https://youtube.com/watch?v=..." className="w-full text-[10px]" />
                </div>
                  ...
                  <div>
                    ...
                    <Select value={commentMax} onValueChange={setCommentMax}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="200" className="text-[10px]">200</SelectItem>
                ...
                <Button className="w-full text-xs" onClick={handleFetchComments}
                  disabled={commentFetching || !commentURL.trim()}>
                  {commentFetching ? 'Fetching...' : 'Fetch Comments'}
                </Button>
                ...
                {allComments.length > 0 && (
                  <>
                    <div className="flex flex-col gap-1">
                      ...
                      <Input value={commentMinLikes} onChange={e => setCommentMinLikes(e.target.value)}
                        type="number" min="0" placeholder="0 = include all"
                        className="w-full text-[10px]" />
                      <p className="text-[9px] text-muted-foreground">
                        {filteredComments.length} of {allComments.length} comments pass filter
                      </p>
                    </div>
                    <div className="flex gap-1">
                      <Button className="flex-1 text-[10px]" size="sm" onClick={runCommentAnalysis}
                        disabled={commentRunning || !filteredComments.length}>
                        {commentRunning ? 'Analysing...' : 'Run AI Analysis'}
                      </Button>
                      <Button variant="outline" className="text-[10px]" size="sm" onClick={exportCommentCSV}>
                        CSV
                      </Button>
                    </div>
                    {commentRunning && <Progress value={undefined} className="h-1 w-full animate-pulse" />}
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col flex-1 overflow-hidden">
          <TabsList className="border-b border-border flex-shrink-0 px-3">
            ...
            {allComments.length > 0 && (
              <TabsTrigger value="commentsraw" className="text-[10px]">
                Raw Comments ({allComments.length})
              </TabsTrigger>
            )}
          </TabsList>

          {/* OUTLIER PATTERNS TAB */}
                ...
                {!outlierResult ? (
                  ...
                  <>
                    ...
                    <ResultSection title="Dominant Patterns">
                      <div className="grid grid-cols-2 gap-2">
                        {[
                          ['Title Type', outlierResult.dominant_title_type],
                          ['Hook Structure', outlierResult.dominant_hook],
                          ['Emotional Trigger', outlierResult.dominant_emotion],
                          ['Pacing', outlierResult.dominant_pacing],
                        ].map(([label, value]) => (
                    ...
                    {/* PATTERN STATEMENT */}
                    <ResultSection title="Pattern Statement">
                      <p className="text-[11px] text-foreground leading-relaxed">{outlierResult.pattern_statement}</p>
                    </ResultSection>
                    ...
                    <ResultSection title="Title Formulas">
                      <div className="flex flex-col gap-2">
                        {outlierResult.title_formulas?.map((f, i) => (
                          <div key={i} className={`px-3 py-2 border border-border text-[10px] text-foreground ${i % 2 === 0 ? 'bg-background' : 'bg-muted/20'}`}>
                            {f}
                          </div>
                        ))}
                      </div>
              ...
              <div className="p-4 max-w-3xl mx-auto flex flex-col gap-4">
                {!commentResult ? (
                  ...
                  <>
                    ...
                    {/* RECURRING QUESTIONS */}
                    <ResultSection title="Recurring Questions from Audience">
                      <p className="text-[9px] text-muted-foreground">Questions the audience repeatedly asks that existing videos don't answer.</p>
                      <div className="flex flex-col gap-1.5 mt-1">
                        {commentResult.recurring_questions?.map((q, i) => (
                    ...
                    {/* CONTENT COMPLAINTS */}
                    <ResultSection title="Content Complaints">
                      <p className="text-[9px] text-muted-foreground">Specific things the audience says existing videos fail to cover.</p>
                      <div className="flex flex-col gap-1.5 mt-1">
                        {commentResult.content_complaints?.map((c, i) => (
                          <div key={i} className={`flex items-start gap-2 px-3 py-2 border border-border text-[10px] ${i % 2 === 0 ? 'bg-background' : 'bg-muted/20'}`}>
                    ...
                    {/* CONTENT GAPS */}
                    <ResultSection title="Content Gaps">
                      <p className="text-[9px] text-muted-foreground">Topics the audience wants that no existing video provides.</p>
                      <div className="flex flex-col gap-1.5 mt-1">
                        {commentResult.content_gaps?.map((g, i) => (
                          <div key={i} className={`flex items-start gap-2 px-3 py-2 border border-border text-[10px] ${i % 2 === 0 ? 'bg-background' : 'bg-muted/20'}`}>
                    ...
                    {/* TITLE IDEAS */}
                    <ResultSection title="Title Ideas from Comments">
                      <p className="text-[9px] text-muted-foreground">Video titles derived directly from what the audience is asking.</p>
                      <div className="flex flex-col gap-1.5 mt-1">
                        {commentResult.title_ideas?.map((t, i) => (
                          <div key={i} className={`px-3 py-2 border border-border text-[10px] text-foreground font-medium ${i % 2 === 0 ? 'bg-background' : 'bg-muted/20'}`}>
                            {t}
                          </div>
                        ))}
                      </div>
                      ...
                      {filteredComments.map((c, i) => (
                        <tr key={c.id} className={`border-b border-border ${i % 2 === 0 ? 'bg-background' : 'bg-muted/20'}`}>
                          ...
                          <td className="px-3 py-2 text-[9px]">
                            <Badge variant={c.isReply ? 'secondary' : 'outline'} className="text-[8px]">
                              {c.isReply ? 'reply' : 'top'}
                            </Badge>
                          </td>
